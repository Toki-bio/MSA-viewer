...
Phase 0 call graph is complete. The scroll-refresh path is surprisingly tight: every function that loops over columns (`generateScale`, `generateScaleHTML`, `addConsensusLine`, `createSequenceLine`) receives the windowed `colStart`/`colEnd` range, not the full alignment width. The only full-width values passed through are `p.len` (used only for `numBlocks` and spacer sizing) and `blockLen` inside `_applyColumnWindowStyle` (used only for the element's declared CSS width, not for iteration). The codon-analysis `.frameShifts[index]` inner loop is per-row (tiny), not per-alignment-width. Next phase: classify every function as SAFE/SUSPECT/UNCLEAR.
