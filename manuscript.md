...
> Round 13 (2026-09-24): After checking a GLM manual audit against the code: the in-browser MAFFT Speed control sent disttbfast -C (thread count), so every setting gave identical alignments; it now sets -E (--retree 1/2/3). The server's /api/mafft is never called by the viewer, so "server-side MAFFT" is dropped from 2.1; Limitations names the FFT-NS strategies.
> Round 12 (2026-09-24): Supplementary list no longer includes the deployment guide or the feature inventory (internal documents). BoxShade given a URL. UGENE's shading described from its current documentation: one adjustable threshold, or four fixed identity bands, in fixed colours.
...
**Supplementary information:** Supplementary Table S1 (comparison with existing tools), a 13-section user manual, full parameter ranges for the clustering, dot plot, and repeat/TSD tools, and example datasets in each supported format are available at the repository.
