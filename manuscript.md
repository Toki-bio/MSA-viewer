...
- **Compact mode**: SVG-based read-packing tracks inspired by IGV. A coverage histogram is displayed at the top. Individual reads are rendered as bars with mismatch positions marked in red. A greedy track-assignment algorithm packs hundreds or thousands of reads into minimal vertical space. Optional controls include "Diffs only" (reads reduced to 4-pixel hairlines, only mismatches visible) and "Pairs" (dashed lines connecting paired-end reads detected from SAM flags).
- **Highlight Diffs**: a cross-mode overlay that dims fully-conserved columns to 25% opacity, drawing attention to variable sites while preserving alignment coordinates in all view modes.
