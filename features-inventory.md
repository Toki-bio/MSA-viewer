...
**Graphical hit-distribution diagram (2026-08-24):** a ruler + one colored bar per hit above the summary table, positioned/sized by query span, colored by NCBI's conventional score-tier bands as a continuous gradient (≥200 red, 80-200 pink, 50-80 green, 40-50 blue, <40 black, interpolated between these anchors rather than 5 flat buckets — hex approximated, not pixel-verified against NCBI's own stylesheet). Hover for a tooltip (description/score/e-value/query range); click scrolls to and flashes the matching table row. Bars render as ~2px hairlines (each row keeps a taller invisible hit-target so hover/click precision doesn't suffer). The diagram lives in its own independently-scrollable section, separate from the summary table below. Ruler shows interior tick labels at a "nice" step, not just the two endpoints. Tooltip is a single shared `position:fixed` element appended to `<body>` (fixed a real clipping bug where a tooltip nested inside the scrollable diagram section got cut off near the container's edge). The results dialog itself is now draggable/resizable/minimizable via the same helper the Clustering Results modal uses. 5 visual concepts were prototyped as real working renders using actual search data (not mockups) — inline table sparklines, a score-vs-position lollipop plot, an identity-intensity heat strip, and a coverage-depth histogram were all built and compared side by side before the user chose the NCBI-style bars.

**Search index caching (2026-08-24):** previously only the raw FASTA text was cached in IndexedDB — the built k-mer inverted index (which only ever lived in a given Worker's own memory) was rebuilt from scratch on every new tab, window, or pool worker, even with a warm text cache. The built index itself is now cached (IndexedDB's structured-clone algorithm supports `Map` and `TypedArray` values natively, no manual serialization needed) — verified 8x faster on a fresh-tab repeat search against the same database (10.6s cold → 1.3s warm), same correct result both times.
