...
Manual 3.4: "... a Web Worker downloads and caches in IndexedDB, so the first query against a database is slower than subsequent ones. Scoring uses Smith-Waterman local alignment."

Where to look: blast-worker.js (whole file is small enough to grep); script.js: grep "blast-worker".
