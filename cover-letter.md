...
**Why it is novel.** Existing MSA tools fall into three groups: installed desktop applications (Jalview, AliView, SeaView, and integrated toolkits such as UGENE and MEGA), browser-based but read-only viewers (MSAViewer), and read-alignment browsers (IGV). We are explicit in the manuscript that UGENE in particular matches ViewAlign on format breadth and on several analyses; our claim is not that ViewAlign has more features than every desktop toolkit, but that this analytical range has not previously been available without installation. The combined editing-to-export workflow described here is what that URL delivers. Key distinguishing features include:
