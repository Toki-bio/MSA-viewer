...
(async () => {
  ...
  const viewportWidth = Number(process.env.VIEWALIGN_VIEWPORT_WIDTH || 1400);
  const zoomSliderValue = process.env.VIEWALIGN_ZOOM_SLIDER;
  const steps = Number(process.env.VIEWALIGN_SCROLL_STEPS || 15);
  const browser = await chromium.launch({ executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe', headless: true });
  const page = await browser.newPage({ viewport: { width: viewportWidth, height: 1080 } });
  const client = await page.context().newCDPSession(page);
  await client.send('Profiler.enable');
  await client.send('Profiler.setSamplingInterval', { interval: 100 });

  await page.goto(`${baseUrl}/?url=/oma_test.fas`, { waitUntil: 'load', timeout: 60000 });
  await page.waitForTimeout(1500);
  if (zoomSliderValue !== undefined) {
  ...
  console.log(`Top self-time functions during ${steps} scroll steps (microseconds):`);
